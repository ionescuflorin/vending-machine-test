...
const ProductListItem = ({ product, balance, onClick }) => (
  <div className="panel panel-default">
    <div className="panel-heading">
      Price: <strong>${product.price}</strong> Count:{' '}
    ...
    <div className="panel-footer">
      {product.count <= 0 ? (
        ...
        <button className="btn btn-primary" onClick={() => onClick(product.id)}>
          Buy
        </button>
      ) : (
        <label className="btn-sm text-warning">
          Load <strong>${product.price - balance.value}</strong> more
        </label>
      )}
    </div>
  </div>
